...
for i in range(1,n,2):
    y1+= (1/np.pi/np.pi)*(np.cos(2 * np.pi * i * x)/i/i) 

# 叠加100条曲线 组成三角波
n =100
y2 = np.zeros_like(x)
for i in range(1,n,2):
    y2+= (1/np.pi/np.pi)*(np.cos(2 * np.pi * i * x)/i/i) 

# 叠加5条曲线 组成三角波
n =5
y3 = np.zeros_like(x)
for i in range(1,n,2):
    y3+= (1/np.pi/np.pi)*(np.cos(2 * np.pi * i * x)/i/i) 

# 叠加19条曲线 组成三角波
n =19
y4 = np.zeros_like(x)
for i in range(1,n,2):
    y4+= (1/np.pi/np.pi)*(np.cos(2 * np.pi * i * x)/i/i) 
# y4 = np.zeros(1000)
